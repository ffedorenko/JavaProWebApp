...
@WebFilter("/Task5")
public class CreateAccessFilter extends HttpFilter implements Filter {
		...
		if (METHOD_GET.equals(httpRequest.getMethod())) {
			...
			if (nonNull(user) && UserRole.ADMIN == user.getUserRole()) {
				...
				httpRequest.setAttribute("errorMessage",
						"Non-authorized user with non-admin role not allowed to access create");
				httpRequest.getRequestDispatcher("/JSP/task5.jsp").forward(httpRequest, response);
			}
		}
